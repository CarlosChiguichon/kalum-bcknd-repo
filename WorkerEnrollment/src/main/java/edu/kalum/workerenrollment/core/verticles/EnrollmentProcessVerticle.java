...
import edu.kalum.workerenrollment.core.client.services.EnrollmentResponse;
...
public class EnrollmentProcessVerticle extends AbstractVerticle {
    ...
    public void start(){
        ...
        CircuitBreaker breaker = CircuitBreaker.create(this.CIRCUIT_BREAKER_ENROLLMENT_PROCESS,vertx,new CircuitBreakerOptions()
                ...
                .halfOpenHandler(circuit -> { logger.warn("Circuit breaker half-opened");});
        ...
        this.eventBus.consumer(this.EVENT_BUS_ENROLLMENT_PROCESS, message -> {
            ...
            breaker.<JsonObject>execute( future -> {
                ...
                try{
                    ...
                    EnrollmentResponse response = enrollmentService.enrollmentProcess(enrollmentRequest);
                    if(response.getStatusCode() == 201){
                        result = new JsonObject().put("status",201).put("message","Enrollment Process success");
                    } else {
                        result = new JsonObject().put("status",403).put("message","Error al procesar la inscripciòn ".concat(gson.toJson(enrollmentProcessRequest)));
                    }
                    logger.info(gson.toJson(response));
                    future.complete(result);
                    logger.info("Enrollment process services success");
                }catch (Exception e){
                    result = new JsonObject().put("status",503).put("message","Error al procesar la inscripciòn ".concat(gson.toJson(enrollmentProcessRequest)));
